...
UINT64* map_page(SEAMRR_PT_CTX* SeamrrPtCtx, UINT64 LinearAddr, UINT64 PhysAddr, UINT64 Attr, PAGE_SIZE PageMappingSize, BOOL IsKeyHoleMapping) {
    ...
    /*LOG("mapping page, pa:0x%lx\tva:0x%lx\n", PhysAddr, LinearAddr);*/
    ...
    for (Idx = 0; Idx < CurNumPageLevels - 1; Idx++) {
        ...
        /*LOG("%s idx:%d", &pt_names[Idx*5], PtIdx);*/
        /*check if PT exists*/
        if (PxeLinear[PtIdx] == 0) {
            ...
            /*LOG("\tno mapping, allocating a %s at :0x%lx\t", &pt_names[(Idx + 1)*5], SeamrrPtCtx->PtAllocatorPa);*/
            SeamrrPtCtx->PtAllocatorPa += SEAMRR_PAGE_SIZE; /*update the next availabe pa for a page table(pml4, pdpt, pd or pt)*/
            /*LOG("next PtAllocatorPa: 0x%lx", SeamrrPtCtx->PtAllocatorPa);*/
        }
        /*LOG("\n");*/
        ...
        /*LOG("next pt level page, pa:0x%lx\tva:0x%lx\n", (ulong)PxePhysical, (ulong)PxeLinear);*/
    }
...
UINT64 map_pseamldr_sysinfo_table(SEAMRR_PT_CTX* SeamrrPtCtx) {
    ...
    if (map_page(SeamrrPtCtx, CurLinAddress, CurPhysAddress, IA32_PG_P | IA32_PG_A | IA32_PG_NX, PAGE_4K, false) == NULL) {
        Status = -1;
        LOG("Failed to map sysinfo table!\n");
        goto EXIT;
    }
...
UINT64 setup_pseamldr_keyhole_region(SEAMRR_PT_CTX *SeamrrPtCtx) {
    ...
    LOG("edit rgn linear address base\n");
    for (Idx = 0; Idx < SeamldrData.PSeamldrConsts->CKeyholeRgnSize / SEAMRR_PAGE_SIZE; Idx++) {
        ...
        if (CurMappedPtPa != PrevMappedPtPa) {
            ...
            LOG("next edit rgn linear address");
        }
        CurLinAddr += SEAMRR_PAGE_SIZE;
    }
...
SEAMRR_PT_CTX* init_pseamldr_pt_ctx(SEAMRR_PT_CTX* SeamrrPtCtx, UINT64 SeamRrVa, UINT64 SeamRrBase, UINT64 SeamRrSize, UINT64 PSeamldrRangeBase, UINT64 PagingStructSize){
    ...
    LOG("SeamrrPtCtx->PtBaseAddrLa\t: 0x%lx\n", SeamrrPtCtx->PtBaseAddrLa);
    LOG("SeamrrPtCtx->PtBaseAddrPa\t: 0x%lx\n", SeamrrPtCtx->PtBaseAddrPa);
    LOG("SeamrrPtCtx->PtAllocatorPa\t: 0x%lx\n", SeamrrPtCtx->PtAllocatorPa);
    LOG("SeamrrPtCtx->PagingStructSize\t: 0x%lx\n", SeamrrPtCtx->PagingStructSize);
    LOG("SeamrrPtCtx->VPDelta\t\t: 0x%lx\n", SeamrrPtCtx->VPDelta);
    return SeamrrPtCtx;
}
...
BOOL load_p_seamldr(){
    ...
    if(get_pseamldr_consts(pseamldr_consts) != 0){
        LOG("get pseamldr consts error\n");
        return false;
    }
    LOG("get pseamldr consts: OK\n\n");
    ...
    LOG("PSysInfoTable NP's va: 0x%lx\n", (UINT64)SeamldrData.PSysInfoTable);
    LOG("SeamldrData.SysInfoTable->PSeamldrRange.Base: 0x%lx\n", SeamldrData.PSysInfoTable->PSeamldrRange.Base);
    LOG("SeamldrData.SysInfoTable->PSeamldrRange.Size: 0x%lx\n", SeamldrData.PSysInfoTable->PSeamldrRange.Size);
    ...
    LOG("SeamldrData.SeamrrVa: 0x%lx\n", SeamldrData.SeamrrVa);
    LOG("SeamldrData.SeamrrVaLimit: 0x%lx\n\n", SeamldrData.SeamrrVaLimit);
    ...
    LOG("loading p_seamldr code ------------------------------------------------\n");
    p_seamldr_size = load_p_seamldr_code();
    if(p_seamldr_size == -1){
        LOG("load_p_seamldr_code error\n");
        return false;
    }
    LOG("loading p_seamldr code: OK\n\n");

    LOG("Init Pseamldr PT Ctx --------------------------------------------------\n");
    ...
    LOG("Init Pseamldr PT Ctx: OK\n\n");
    
    LOG("relocating image ------------------------------------------------------\n");
    ...
    if (status != 0) {
        LOG("Failed to relocate P-Seamldr\n");
        return false;
    };
    LOG("relocating image: OK\n\n");

    LOG("mapping pseamldr code pages -------------------------------------------\n");
    status = map_pseamldr_code_region(&SeamrrPtCtx, p_seamldr_size);
    if (status != 0) {
        LOG("Failed to map pseamldr code pages\n");
        return false;
    };
    LOG("mapping pseamldr code pages: OK\n\n");

    LOG("mapping pseamldr stack pages ------------------------------------------\n");
    status = map_pseamldr_stack_region(&SeamrrPtCtx);
    if (status != 0) {
        LOG("Failed to map pseamldr stack pages\n");
        return false;
    };
    LOG("mapping pseamldr stack pages: OK\n\n");

    LOG("setup keyhole region --------------------------------------------------\n");
    status = setup_pseamldr_keyhole_region(&SeamrrPtCtx);
    if(status != 0){
    LOG("Failed to setup keyhole region\n");
        return false;
    };
    LOG("setup keyhole region: OK\n\n");

    LOG("mapping pseamldr data pages -------------------------------------------\n");
    status = map_pseamldr_data_region(&SeamrrPtCtx);
    if (status != 0) {
        LOG("Failed to map pseamldr data pages\n");
        return false;
    };
    LOG("mapping pseamldr data pages: OK\n\n");

    LOG("mapping pseamldr sysinfo table ----------------------------------------\n");
    status = map_pseamldr_sysinfo_table(&SeamrrPtCtx);
    if(status != 0){
        return false;
    }
    LOG("mapping pseamldr sysinfo table: OK\n\n");

    LOG("mapping pseamldr tdxmodule region -------------------------------------\n");
    status = map_pseamldr_tdxmodule_region(&SeamrrPtCtx);
    if(status != 0){
        LOG("Failed to map pseamldr tdxmodule region\n");
        return false;
    }
    LOG("mapping pseamldr tdxmodule region: OK\n\n");

    LOG("setting up sysinfo table\n");
    setup_sysinfo_table();

    LOG("SeamldrData.PSeamldrConsts->CCodeRgnSize:0x%lx\n", SeamldrData.PSeamldrConsts->CCodeRgnSize);



    LOG("ending NP-SEAMLDR ... .....................................................\n");
    return true;
}
