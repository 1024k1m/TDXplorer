...
uint64_t RelocateImage(uint64_t ElfImage, uint64_t RelocationAddr) {
    ...
    for (uint32_t i = 0; i < ElfHeader->e_shnum; i++) {      //  e_shnum- # of section header entries
        ...
        LOG("section header num: 0x%xtype: 0x%x", i, CurSheader->sh_type);
