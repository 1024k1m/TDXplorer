...
class EFlagsManager {
    ...
    public:
        ...
        std::set<unsigned long> SolveConstraint(std::set<KVExprPtr> constraints, std::string s_name);
        // SYMemObject *GetSymbol(std::string sname);

        uint64_t ConcretizeExpression(KVExprPtr exprPtr, bool use_symbuf = false);
        uint64_t EvalCondition(entryID insnID, uint64_t addr, uint64_t b1, uint64_t b2) ;
        uint64_t GetSymAddrConcreteBase(KVExprPtr exprPtr);
        ...
        KVExprPtr getLastFlgExpression() {return m_LastExpr;};
