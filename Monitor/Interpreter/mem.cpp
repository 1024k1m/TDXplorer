...
bool SYMemState::sendSeamMemAccessReq(SERVREQ req, ulong seam_va, long &data, ulong size){

    khole_access++;
    sreq->req = req;
    sreq->seam_va = seam_va;
    sreq->data_size = size;

    /*std::cout << "sendSeamMemAccessReq\n";*/
    if(req == SERVREQ_WRITE_MEM){
        sreq->data = data;
    }
...
bool SYMemState::writeConcreteValue(ulong address, ulong size, long v) {
    ...
    std::cout << "writeConcreteValue" << std::endl;
    if((address >= sreq->khole_start) && (address < (sreq->khole_start + sreq->khole_size))){
        return sendSeamMemAccessReq(SERVREQ_WRITE_MEM, address, v, size);
    }
    non_khole_access++;
...
bool SYMemState::readConcreteValue(ulong address, ulong size, long &v) {
    ...
    // std::cout << "readConcreteValue at 0x" << std::hex << address<< std::endl;
    if((address >= sreq->khole_start) && (address < (sreq->khole_start + sreq->khole_size))){
        return sendSeamMemAccessReq(SERVREQ_READ_MEM, address, v, size);
    }
