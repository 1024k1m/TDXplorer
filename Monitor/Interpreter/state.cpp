...
bool VMState::createSYMemObject(ulong addr, ulong size, bool isSigned, bool hasSeed, long conVal, const char *name) {
    ...
    std::cout << "#-----------------------conVal: 0x" << std::hex << conVal << std::endl;
    ...
    std::cout << "2conVal: 0x" << std::hex << obj->i64 << std::endl;
    ...
    for (auto it : m_SYMemObjects)
    {
        //print symbols, and seed values 
        std::cout << "s name :" << it.second->name << " ,hasseed : " << it.second->has_seed  << std::endl;
        switch(it.second->size)
        {
            case 1:
                std::cout << "1val:0x " << std::hex << it.second->i8 << std::endl;
                break;
            case 4:
                std::cout << "2val:0x " << std::hex << it.second->i32 << std::endl;
                break;
            case 8:
                std::cout << "4val:0x " << std::hex << it.second->i64 << std::endl;
                break;
        }
    }
...
VMState::SYMemObject* VMState::GetSymMemObject(std::string sname){

    struct SYMemObject *sym;
...
VMState::SYRegObject* VMState::GetSymRegObject(std::string sname){

    struct SYRegObject *sym;

    for (auto it : m_SYRegObjects){
        if(it.second->name == sname){
            sym = it.second;
        }
    }
