    ...
    if (!dependFlagCon(in, bExecute))
    {
        ...
        if (bPath_explore) //in path exploration
        {
            ...
            if(BtoTake == trueB) {
                ...
                endCurrentPath = true;
                endCurrentPathReason = PATH_IMPOSSIBLE;
            }
...
bool CThinCtrl::processFunction(ulong status){
    ...
    while (true) { /*while loop start*/
        ...
        if (execProfile->executionMode == EXEC_MD_START_PATH_SEARCH_AT_INS_COUNT && execData->insn_count == execProfile->startIncCount) {
            ...
            assert(execData->current_path < 1000);
        ...
        std::cout << "SE: rip: 0x" << std::hex << m_regs->rip << " in: " << in->format() << std::endl;
        ...
        switch (cate) {
            ...
            {
                branch_count++;
                // std::cout << "debug_log c_BranchInsn : rip :0x" << (m_regs->rip - in->size()) << std::endl;
        ...
        if(execProfile->executionMode == EXEC_MD_START_PATH_SEARCH_AT_INS_COUNT){
                if(execProfile->terminate_ins_count && (execData->insn_count >= execProfile->terminate_ins_count)){
                    endCurrentPath = true;
                }
                if(execData->is_next_ins_seamret == true){
                    endCurrentPath = true;
                    execData->is_next_ins_seamret = false;
                }
                if(endCurrentPath){
                    std::cout << "\nEnding Current path -----------------------------------------" << std::endl;
    ...
    if(in->getOperation().getID() == e_int3){
        while(tdx_sp_ins[count].size != 0 && count < TDXMODULE_SPECIAL_INS_COUNT){
            if(tdx_sp_ins[count].va == rip){
                ...
                /*current dyninst can not decode seamret, movdir64b, may be more, so the format printed below is wrong.*/
                // std::cout << "special in: " << sp_in->format() << std::endl;
                // std::cout << "count:" << std::dec << count << " size:" << tdx_sp_ins[count].size << std::endl;
                int i = 0;
                while(i < tdx_sp_ins[count].size){
