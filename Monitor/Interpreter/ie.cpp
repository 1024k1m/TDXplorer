...
bool SymExecutor::run(VMState *vm) {
    ...
    for (auto IOI : m_IOIs) {
        ...
        switch (id) {
            ...
            case e_bts: {
                ret = process_bts(vm, IOI);
                break;
            }
            case e_btr: {
                ret = process_btr(vm, IOI);
                break;
            }
            case e_adc: {
                ret = process_adc(vm, IOI);
                break;
            }
...
bool SymExecutor::_run_prologue(void) {return true;}
...
bool SymExecutor::process_bt(VMState *vm, InstrInfoPtr &infoptr) {
    ...
    OprndInfoPtr &oidst = vecOI[0];
    OprndInfoPtr &oisrc = vecOI[1];
    KVExprPtr e1, e2, er, temp1, temp2, temp3;
    long v1, v2 ;
    bool res ;
    ...
    if(oisrc->symb) {
        assert(0); //consider a concrete bit position
    } else {
         res = oisrc->getConValue(v1) ;
         assert(res);
         e1.reset(new ConstExpr(v1, oidst->size, 0)) ;
    }
    ...
    if(oidst->symb) {
        ...
        assert(0); //if both operands are conc, no need to SE, just emulate here
    }

    /*now set the flags*/
    temp1.reset(new ConstExpr(1, oidst->size, 0)); /*1 as a const*/
    temp2.reset(new Shl_SalExpr(temp1, e1)); /*1 << src*/

    temp3.reset(new AndExpr(e2, temp2));
    er.reset(new ShrExpr(temp3, e1));
    vm->SaveFlagChangingInstructionExpr(e_bt, er) ;
    return true ;
}

bool SymExecutor::process_bts(VMState *vm, InstrInfoPtr &infoptr) {
    auto &vecOI = infoptr->vecOI;
    OprndInfoPtr &oidst = vecOI[0];
    OprndInfoPtr &oisrc = vecOI[1];
    KVExprPtr e1, e2, er, ed;
    long v1, v2 ;
    bool res ;
    KVExprPtr temp1, temp2, temp3;
    ...
    if(oidst->symb) {
        /*set the destination bit identified by offset
        dest = dest | (1 << src) */        
        temp1.reset(new ConstExpr(1, oidst->size, 0)); /*1 as a const*/
        temp2.reset(new Shl_SalExpr(temp1, e1)); /*1 << src*/
        ed.reset(new OrExpr(e2, temp2)); /*dest | (1 << src)*/
        std::cout << "dst expr: ";
        ed->print();
        oidst->setSymValue(vm, ed);

        /*now set flags*/
        temp3.reset(new ShrExpr(e2, e1));
        er.reset(new AndExpr(temp3, temp1));
        std::cout << "cf expr: ";
        er->print();
        vm->SaveFlagChangingInstructionExpr(e_bts, er) ;
    }
    else {
        /*no symbols, so we can concretely emulate*/
        // assert(0);
        /*if using this, fix this to update the flags accordingly.*/

        assert(oidst->opty == OPTY_MEMCELLCON); /* as of now We expect the dst to be conc mem val*/
        // std::cout << "v2: 0x " << std::hex << v2 << std::endl;
        v2 = v2 | (1UL << v1);
        std::cout << "v2: 0x " << std::hex << v2 << std::endl;

        MemValue MV;
        MV.addr = oidst->mem_conaddr;
        MV.size = oidst->size;
        MV.isSymList = false;
        res = vm->readMemory(MV);
        assert(res);
        assert(!MV.bsym);
        std::cout << "adr: 0x " << std::hex << oidst->mem_conaddr << std::endl;
        MV.i64 = v2;
        res = vm->writeMemory(MV);
        assert(res);

        /*set the flags:cf*/
        ia32_rflags_t eflags = {0};

        FLAG_STAT flaguc = FLAG_UNCERTAIN;
        FLAG_STAT flag_zf;
        eflags.zf = (vm->getFlagBit(x86_64::zf, flag_zf) != FLAG_UNCERTAIN) ? flag_zf: 0;
        eflags.cf = (v2 >> v1) & 0x1;
        set_concrete_eflags(vm, eflags.raw);
    }

    return true ;
}

bool SymExecutor::process_btr(VMState *vm, InstrInfoPtr &infoptr) {
    auto &vecOI = infoptr->vecOI;
    OprndInfoPtr &oidst = vecOI[0];
    OprndInfoPtr &oisrc = vecOI[1];
    KVExprPtr e1, e2, er, ed;
    long v1, v2 ;
    bool res ;
    KVExprPtr temp1, temp2, temp3, temp4;
    std::cout << "at btr()\n";
    ...
    if(oisrc->symb || oidst->symb) {
        std::cout << "at btr() 3\n";

        /*clear the destination bit identified by offset
        temp1.reset(new ConstExpr(1, oidst->size, 0)); /*1 as a const*/
         std::cout << "at btr() 3.1\n";
       
        temp2.reset(new Shl_SalExpr(temp1, e1)); /*1 << src*/
        std::cout << "at btr() 3.2\n";
        temp3.reset(new NotExpr(temp2)); /*~(1 << src)*/ 
        std::cout << "at btr() 3.3\n";
        ed.reset(new AndExpr(e2, temp3)); /*dest | (1 << src)*/
        std::cout << "dst expr: ";
        ed->print();
        oidst->setSymValue(vm, ed);
 
        /*now set flags*/
        temp4.reset(new ShrExpr(e2, e1));
        er.reset(new AndExpr(temp4, temp1));
        std::cout << "cf expr: ";
        er->print();
        vm->SaveFlagChangingInstructionExpr(e_btr, er) ;
        ...
        assert(!MV.bsym);
        std::cout << "adr: 0x " << std::hex << oidst->mem_conaddr << std::endl;
        std::cout << "val: 0x " << std::hex << MV.u64 << std::endl;
        MV.i64 = v2;
        res = vm->writeMemory(MV);
        assert(res);
...
bool SymExecutor::process_mov(VMState *vm, InstrInfoPtr &infoptr) {
    ...
    if(oisrc->symb) {
        ...
        if(oidst->opty == OPTY_MEMCELLSYM){
            MemValue MV = {oidst->mem_conaddr, oidst->size};
            res = vm->readMemory(MV);
            assert(res);
        }
    }
...
bool SymExecutor::process_xadd(VMState *vm, InstrInfoPtr &infoptr) {
    ...
    if (oisrc1->symb && oisrc2->symb) {
        ...
        res = oisrc1->setConValue(vm, v1 + v2);
        assert(res);

        ulong eflags = (ulong)calculate_eflags_add(v1, v2, oidst->size).raw;
        set_concrete_eflags(vm, eflags);
        std::cout << "rflags:" << eflags << std::endl;
...
bool SymExecutor::process_test(VMState *vm, InstrInfoPtr &infoptr) {
    ...
    if (oisrc1->symb && oisrc2->symb) {
        ...
        /*we encounter this branch in _SYM_ADDR mode*/
#ifndef _SYM_ADDR
        ERRR_ME("Unexpected operands");
        assert(0);
#endif
        ...
        ulong eflags = (ulong)calculate_eflags_test(v1, v2, oidst->size).raw;
        set_concrete_eflags(vm, eflags);
        std::cout << "rflags:" << eflags << std::endl;
...
bool SymExecutor::process_cmp(VMState *vm, InstrInfoPtr &infoptr) {
    ...
    if (oisrc1->symb && oisrc2->symb) {
        ...
        std::cout << "dst : " << std::hex << v2 << " dst sz: 0x" << oisrc2->size << std::endl;
        if(oisrc1->size > oisrc2->size){
            assert(oisrc2->size != 8);
            v2 = signext_to_long(oisrc2->size, v2);
        }
        std::cout << "dst aft sign extend : " << std::hex << v2 << std::endl; 
        ExprPtr c2(new ConstExpr(v2, oisrc1->size, 0)); 
        ...
        ulong eflags = (ulong)calculate_eflags_cmp(v1, v2, oidst->size).raw;
        set_concrete_eflags(vm, eflags);
        std::cout << "rflags:" << eflags << std::endl;
...
bool SymExecutor::set_concrete_eflags(VMState *vm,  ulong flgs){

        bool res;
        ia32_rflags_t eflags;
        eflags.raw = flgs;
        FLAG_STAT flaguc = FLAG_UNCERTAIN;
        FLAG_STAT flagset = FLAG_SET;
        FLAG_STAT flagclr = FLAG_CLEAR;

        vm->clearAllSymFlag();
        res = vm->setFlagBit(x86_64::cf, eflags.cf ? flagset : flagclr);
        assert(res);
        res = vm->setFlagBit(x86_64::pf, eflags.pf ? flagset : flagclr);
        assert(res);
        res = vm->setFlagBit(x86_64::af, eflags.af ? flagset : flagclr);
        assert(res);
        res = vm->setFlagBit(x86_64::zf, eflags.zf ? flagset : flagclr);
        assert(res);
        res = vm->setFlagBit(x86_64::sf, eflags.sf ? flagset : flagclr);
        assert(res);
        res = vm->setFlagBit(x86_64::of, eflags.of ? flagset : flagclr);
        assert(res);
        
        return true;
}

ia32_rflags_t SymExecutor::calculate_eflags_test(int value1, int value2, int size) {
    ia32_rflags_t eflags = {0}; // Initialize all flags to false

    // Simulate bitwise AND: result = value1 & value2
    int result = 0;
    unsigned int max_val = 0;

    if (size == 1) {
        // 8-bit test (1 byte)
        result = (value1 & 0xFF) & (value2 & 0xFF);
        max_val = 0xFF;
    } else if (size == 2) {
        // 16-bit test (2 bytes)
        ...
        // 32-bit test (4 bytes)
        result = (value1 & 0xFFFFFFFF) & (value2 & 0xFFFFFFFF);
        max_val = 0xFFFFFFFF;
    } else {
        throw std::invalid_argument("Unsupported size. Use 1, 2, or 4.");
    }

    // Zero Flag (ZF): Set if the result was zero
    eflags.zf = (result == 0);

    // Sign Flag (SF): Set if the result is negative (sign bit of the result)
    eflags.sf = (result & (1 << (size * 8 - 1))) != 0;

    // Overflow Flag (OF): "Test" doesn't affect signed overflow, so it's always clear.
    eflags.of = 0;

    // Auxiliary Carry Flag (AF): Set if there was a carry from the lower nibble (BCD)
    int lower_nibble1 = value1 & 0xF;
    int lower_nibble2 = value2 & 0xF;
    eflags.af = (lower_nibble1 & lower_nibble2) > 0xF;

    // Parity Flag (PF): Set if the number of set bits in the result is even
    std::bitset<32> result_bits(result & max_val);
    eflags.pf = (result_bits.count() % 2 == 0);

    return eflags;
}

ia32_rflags_t SymExecutor::calculate_eflags_add(int value1, int value2, int size){

    ia32_rflags_t eflags = {0}; // Initialize all flags to false

    // Simulate addition: result = value1 + value2
    int result = 0;
    unsigned int max_val = 0;

    if (size == 1) {
        // 8-bit addition (1 byte)
        result = (value1 & 0xFF) + (value2 & 0xFF);
        max_val = 0xFF;
    } else if (size == 2) {
        // 16-bit addition (2 bytes)
        result = (value1 & 0xFFFF) + (value2 & 0xFFFF);
        max_val = 0xFFFF;
    } else if (size == 4) {
        // 32-bit addition (4 bytes)
        result = (value1 & 0xFFFFFFFF) + (value2 & 0xFFFFFFFF);
        max_val = 0xFFFFFFFF;
    } else {
        throw std::invalid_argument("Unsupported size. Use 1, 2, or 4.");
    ...
    // Carry Flag (CF): Set if there was a carry (unsigned overflow)
    eflags.cf = (result > max_val);
    ...
    // Overflow Flag (OF): Set if there was a signed overflow
    if (size == 1 || size == 2 || size == 4) {
        bool sign1 = value1 & (1 << (size * 8 - 1));
        bool sign2 = value2 & (1 << (size * 8 - 1));
        bool sign_result = result & (1 << (size * 8 - 1));
        // Signed overflow occurs if the signs of the operands are the same, but the sign of the result is different
...
ia32_rflags_t SymExecutor::calculate_eflags_cmp(int value1, int value2, int size){

    ia32_rflags_t eflags = {0}; // Initialize all flags to false

    // Simulate subtraction: result = value1 - value2
    int result = 0;
    unsigned int max_val = 0;

    if (size == 1) {
        // 8-bit comparison (1 byte)
        result = (value1 & 0xFF) - (value2 & 0xFF);
        max_val = 0xFF;
    } else if (size == 2) {
        // 16-bit comparison (2 bytes)
        result = (value1 & 0xFFFF) - (value2 & 0xFFFF);
        max_val = 0xFFFF;
    } else if (size == 4) {
        // 32-bit comparison (4 bytes)
        result = (value1 & 0xFFFFFFFF) - (value2 & 0xFFFFFFFF);
        max_val = 0xFFFFFFFF;
    } else {
        throw std::invalid_argument("Unsupported size. Use 1, 2, or 4.");
    ...
    // Carry Flag (CF): set if there was a borrow (unsigned comparison)
    eflags.cf = (result < 0);
    ...
    // Overflow Flag (OF): set if signed subtraction overflows
    if (size == 1 || size == 2 || size == 4) {
    ...
    // Auxiliary Carry Flag (AF): set if there was a borrow in the lower nibble (BCD)
    int lower_nibble1 = value1 & 0xF;
    int lower_nibble2 = value2 & 0xF;
    eflags.af = (lower_nibble1 < lower_nibble2);
...
bool SymExecutor::process_adc(VMState *vm, InstrInfoPtr &infoptr) {
    ...
    KVExprPtr cf_expr(nullptr);
    if(cf_bit == FLAG_UNCERTAIN){
        KVExprPtr cf_ex = vm->m_EFlagsMgr->getLastFlgExpression();
        std::cout << "cf_expr.size:" << cf_ex->size << std::endl;
        cf_ex->print();
        std::cout << std::endl;

        /*sdjust the expr size*/
        if(cf_ex->size > oidst->size){
            cf_expr.reset(new ExtractExpr(cf_ex, 0, oidst->size)); //, oidst->size, 0));
        }
        else if(cf_ex->size < oidst->size){
            /*TODO: concat leading zeros*/
            assert(0);
        }
        // cf_expr->setExprSize(oidst->size);
        std::cout << "cf_expr.size:" << cf_expr->size << std::endl;
    ...
    if (oisrc1->symb && oisrc2->symb) {
        ...
        if(cf_bit != FLAG_CLEAR){
            res = oisrc1->getSymValue(e0);
            assert(res);

            ExprPtr cf(nullptr);
            if(cf_bit == FLAG_SET){
                cf.reset(new ConstExpr(1, oisrc1->size, 0));
            }
            else if(cf_bit == FLAG_UNCERTAIN){
                cf = cf_expr;
            }
            else{
                assert(0);
        ...
        if(cf_bit != FLAG_CLEAR){
            ...
            if(cf_bit == FLAG_SET){
                v1 += 1;
                c1.reset(new ConstExpr(v1, oisrc2->size, 0));
            }
            else if(cf_bit == FLAG_UNCERTAIN){
                cf = cf_expr;
                ExprPtr c2(new ConstExpr(v1, oisrc2->size, 0));
                c1.reset(new AddExpr(c2, cf, oisrc1->size, 0));
            }
            else {
                assert(0);
            }
        ...
        if(cf_bit == FLAG_UNCERTAIN){
            ...
            ExprPtr c2(new ConstExpr(v1, oisrc2->size, 0));
            oe.reset(new AddExpr(c2, cf_expr));
            res = oidst->setSymValue(vm, oe);
            assert(res);
        }
        else{
    ...
    res = vm->SaveFlagChangingInstructionExpr(e_adc, oe) ;
    assert (res) ;
    return true ;
}
...
bool SymExecutor::process_sbb(VMState *vm, InstrInfoPtr &infoptr) {
    ...
    /*For the moment we expect the cf_bit to be non symbolic*/
    assert(cf_bit != FLAG_UNCERTAIN);
...
bool SymExecutor::process_and(VMState *vm, InstrInfoPtr &infoptr) {
    ...
    std::cout << "oidst->size: " << oidst->size << std::endl;
    std::cout << "oisrc2->size: " << oisrc2->size << std::endl;

    /*and $0x45, %rax  --> makes the rax bytes 4-7 zero*/
    if (oidst->size == 4)  {
        if ((oidst->opty & OPTY_REG) == OPTY_REG) {
            // we need to clear the 3rd byte, according to dyn_reg.h::435
            uint reg_indx = oidst->reg_index ;
...
bool SymExecutor::process_shl_sal(VMState *vm, InstrInfoPtr &infoptr) {
    ...
    if (!oisrc1->symb && !oisrc2->symb) {
        ...
        return false ;
#endif
        /*now concretize and update operand 2*/
        KVExprPtr ex1(nullptr);
        res = oisrc2->getSymValue(ex1);
        assert(res);

        std::cout << "shl_sal shift val is sym: expr :"  << std::endl;
        ex1->print();
        std::cout << std::endl;
        ulong concrete_val = (ulong)vm->m_EFlagsMgr->ConcretizeExpression(ex1);
        std::cout << "Concretized val : " << std::hex << concrete_val << std::endl;
        // assert(0);
        ...
        if(oisrc1->symb){
            ...
            ExprPtr c2(new ConstExpr(concrete_val, oisrc1->size, 0));
            oe.reset(new Shl_SalExpr(e1, c2));
            res = oidst->setSymValue(vm, oe);
            assert(res);
        }
        else{
            long val;
            res = oisrc1->getConValue(val);
            assert(res);
            // std::cout << "reg val to shift: " << std::hex << val << std::endl;
            ExprPtr e1(new ConstExpr(val, oisrc1->size, 0));

            KVExprPtr c2(nullptr);
            res = oisrc2->getSymValue(c2);
            assert(res);
            oe.reset(new Shl_SalExpr(e1, c2));
            val = val << concrete_val;
            res = oisrc1->setConValue(vm, val);
            assert(res);
        }
    }
    else{
...
ulong SymExecutor::isUsingFSGS(VMState* vm, DAPIInstrPtr& I)
{
...
bool SymExecutor::_parseOperand_RX(VMState *vm, DAPIInstrPtr& I, OprndInfoPtr &oi) {
    ...
    if (O->isRead()) {
        ...
        ulong fsgs_base = isUsingFSGS(vm, I); 
        ...
        if (rdwrRegs.size() == 0) {  // Direct memory access
            ...
            if (hasSymReg) {
                ...
                if (exps.size() == 1) {
                    ...
                    concrete_reg_val = vm->m_EFlagsMgr->ConcretizeExpression(exprPTR, true);
