...
uint64_t HistoryManager::getExecAddress (uint64_t address, uint64_t branch1, uint64_t branch2) {
    ...
    if(!left) {
        ...
        if(findInRunpath(branch1)) {
            ...
            std::cout << "PUSHb L: 0x" << std::hex << branch1 << std::endl;
            runpath.push_back(0) ;
            std::cout << "PUSHb L: 0x" << std::hex << 0 << std::endl;
    ...
    if(!right) {
        ...
        if(findInRunpath(branch2)) {
            ...
            std::cout << "PUSHb R: 0x" << std::hex << branch2 << std::endl;
            runpath.push_back(0) ;
            std::cout << "PUSHb R: 0x" << std::hex << 0 << std::endl;
            return 0 ;
        }
...
bool HistoryManager::print(void) {
    ...
    std::cout << "\npaths all:\n" ;
    int path = 0;
    it_c=all_constraint.begin() ; 
    for(auto it_path=all_runpaths.begin() ; 
        (it_c != all_constraint.end()) && (it_path != all_runpaths.end()); 
        it_path++, it_c++) {
        
        assert(path < path_end_reasons.size());
        std::cout << path_end_reasons[path] << ", ";
    ...
    std::cout << "\npath count :" << std::dec << path << std::endl ;
    return true;
}
