...
std::map<std::string, unsigned long long> Z3Handler::Z3SolveOne(std::set<KVExprPtr> constraints){
    ...
    switch (g_solver.check()){
        ...
        case unknown:{
                ...
                printf("\033[47;31m Oops, constraint solving state is unknown ... ! \033[0m\n");
                break;
                }
    }
    return ret;
...
uint64_t Z3Handler::Z3SolveConcritizeToConstant(std::vector<VMState::SYMemObject*> symobjs_all, std::set<KVExprPtr> constraints){
    ...
    for (auto symobj : symobjs_all) {
        if (symobj->has_seed == true){
            std::cout << "symobj val 0x" << symobj->i64 << std::endl;
            ...
            case 4: { // 4 bytes
                if (symobjs[i]->is_signed) {
                    ...
                    for (auto it = symObjectsMap.begin(); it != symObjectsMap.end(); it ++){
                        if (it->first == symobjs[i]) {
                            expr sym_expr = it->second;
                            std::cout << "4-expr-bfr: " << exprs << std::endl;
                            tmp_expr = Z3Handler::Z3ExpressionEvaluatorToConstant(exprs, sym_expr, value_expr);
                            exprs = tmp_expr;
                            std::cout << "4-expr-aft: " << exprs << std::endl;
                            break;
                        }
                    }
                }
            ...
            case 8: { // 8 bytes
                if (symobjs[i]->is_signed) {
                    ...
                    for (auto it = symObjectsMap.begin(); it != symObjectsMap.end(); it ++){
                        if (it->first == symobjs[i]) {
                            expr sym_expr = it->second;
                            // std::cout << "8-expr-bfr: " << exprs << std::endl;
                            tmp_expr = Z3Handler::Z3ExpressionEvaluatorToConstant(exprs, sym_expr, value_expr);
                            exprs = tmp_expr;
                            // std::cout << "8-expr-aft: " << exprs << std::endl;
                            break;
                        }
                    }
                }
    ...
    std::cout << "final expr: " << exprs << std::endl;
    ret = exprs.simplify().get_numeral_uint64();
    std::cout << "final val: " << ret << std::endl;
...
bool Z3Handler::Z3SolveConcritize(std::vector<VMState::SYMemObject*> symobjs_all, std::set<KVExprPtr> constraints){
    ...
    for (auto symobj : symobjs_all) {
        if (symobj->has_seed == true)
            symobjs.push_back(symobj); 
            std::cout << "symobj->name:" << symobj->name << " symobj->i64: " << symobj->i64 << " sz: " << symobj->size << std::endl;
    }

    // std::cout << "symObjectsMap.size: " << symObjectsMap.size() << std::endl;
    ...
    expr tmp_expr = exprs;
    for (int i = 0; i < symobjs.size(); i++){
        // std::cout << "i: " << i << " symobjs[i]->size: " << symobjs[i]->size << std::endl;
        // checking whether the input obj exists in the corrent constraints, raise an error if no;
        ...
        switch (symobjs[i]->size){
            case 1: { // 1 bytes
                if (symobjs[i]->is_signed) {
                    expr value_expr = context_.bv_val(symobjs[i]->i8, 1 * 8);
                    for (auto it = symObjectsMap.begin(); it != symObjectsMap.end(); it ++){
                        std::cout << it->first << " " << symobjs[i] << std::endl;
                        if (it->first == symobjs[i]) {
                            expr sym_expr = it->second;
                            // std::cout << "1-expr-bfr: " << exprs << std::endl;
...
z3::expr Z3Handler::Z3HandlingExprPtr(ExprPtr ptr){
    ...
    switch (kind){
        ...
        case Expr::Kind::Bt:{
            BtExpr * expr_r = static_cast<BtExpr*>(ptr.get());
            ExprPtr R = expr_r->getExprPtrR();
            ExprPtr L = expr_r->getExprPtrL();
            return Z3HandleBt(R, L);
        }
        case Expr::Kind::LShr:{
...
z3::expr Z3Handler::Z3HandleBt(ExprPtr r /*bit position*/, ExprPtr l /*base*/){
    r->print();
    std::cout << "\nZ3HandleBt" << std::endl;
    // assert(0);

    // BtExpr *bt_expr = static_cast<BtExpr*>()
    // expr ret ; //= z3::Bt(Z3HandlingExprPtr(r), Z3HandlingExprPtr(l));
    ConstExpr *const_expr = static_cast<ConstExpr*>(r.get());
    int ofst = const_expr->getValue();
    std::cout << "ofst: " << ofst << std::endl;
    expr x = Z3HandlingExprPtr(l);
    expr y = x.extract(ofst, ofst);
