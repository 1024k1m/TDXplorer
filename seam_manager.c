...
extern void launch_interpreter();
...
void do_tdxcall(ulong seamcall);
...
void do_sanity_checks(){	
	...
	if(!movdir64b_support){
		LOG("CPU does not support MOVDIR64B\n");
		exit(0);
	}
#endif
	...
	for(idx = 0; idx <= 511; idx++){
		if((tdx_sw_pml4[idx] & PDE64_PRESENT) != 0){
			if((s_emulator_pml4[idx] & PDE64_PRESENT) != 0){
				...
				/*fuse now*/
				LOG("Fusing tdx sw PDPT with s_smulator PML4, pml4 idx: %d\n", idx);
...
void debug_info(){
				...
				LOG("rbp:0x%lx rax:0x%lx rbx:0x%lx rcx:0x%lx rdx:0x%lx\n", (ulong)regs.rbp, (ulong)regs.rax, (ulong)regs.rbx, (ulong)regs.rcx, (ulong)regs.rdx);
				LOG("r11:0x%lx r12:0x%lx r13:0x%lx r14:0x%lx \n", (ulong)regs.r11, (ulong)regs.r12, (ulong)regs.r13, (ulong)regs.r14);
				LOG("rflags:0x%lx \n", (ulong)regs.rflags);
...
void handle_seam_log_request(CODE type){
	...
	break;
	default:
		LOGR("unhandled log type received from SEAM\n");
		break;
	}
}
...
void log_active_keyhole_mappings(){
	...
	Since we enable 4 LPs(, our tdxconfigurable) )mod only uses a khole edit region of size 128*4*8B
	The rest of the keyhole edit pages remain unused.*/
	lp = 0;
	while(lp < NUM_ADDRESSIBLE_LPS){
...
void handle_hcall(){
	...
	switch (com->hcall_no)
	{
		...
		{
			com->last_seamcall.return_status = get_saved_reg64(RAX);
			LOG("########## %04lu ", seamcall_count);
			print_seamcall_name();
			LOG("SEAMRET status: %lx %s -----------------\n", com->last_seamcall.return_status, 
								(get_saved_reg64(RAX) == SEAMCALL_SUCCESS) ? "SUCCESS" : "FAIL");
			seamcall_count++;
			if(get_saved_reg64(RAX) != SEAMCALL_SUCCESS){
				LOG("exitting now ...\n");
				exit(0);
			}
			if(com->last_seamcall.tdxcall == TDH_MEM_SEPT_RD){
				print_sept_rd_result();
			}
		...
		default:
		{
			...
			/*read_pseamldr_data_t();
			debug_info();
			LOG("arbitrary HLT for debugging, exitting\n");
			exit(0);*/
	#endif
			debug_info();
			exit(0);
	...
	for (;;) {
		...
		switch (vcpu->kvm_run->exit_reason) {
			case KVM_EXIT_HLT:
			{
				/*debug_info();*/
				handle_hcall();
				if(com->hcall_no == HCALL_SEAMRET && com->last_seamcall.return_status == SEAMCALL_SUCCESS){
			...
			default:{
				LOG("unhandled exit reason ... :0x%x \n", vcpu->kvm_run->exit_reason);
				debug_info();
				exit(1);
			}
		}
...
void install_tdx_module_epilogue(){
	...
	/*prepare com area for Interpreter seam agent*/
...
void seam_manage(){
	...
	com->se.interpreter_pt_updates_done = false;
	...
	com->current_sw = SEAM_SW_TDXMODULE; /*Requests Interpreter manager to get seam mappings for Interpreter*/
	LOG("seam_manager's wait for Interpreter PT update: START\n");
	while (true)
	{
		if(com->se.Interpreter_pt_updates_done == true)
			break;
	}
	LOG("seam_manager's wait for Interpreter PT update: END\n");
...
int main()
{
	...
	LOG("com->pseamldr_ins:0x%lx\n", (ulong)&com->pseamldr_ins);
	...
	switch (pid) {
		...
		case 0: { /*child*/
			...
			LOGR("Interpreter init done, child exiting ...\n");
			exit(EXIT_SUCCESS);
		}
		default: { /*parent*/
			LOGR("executing SEAM environment\n\n");
			seam_manage();
			wait(0);
